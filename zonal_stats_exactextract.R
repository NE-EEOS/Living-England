...
zonal.stats.exactextract <- function(segmentation, list.rasters, tiles=1)
{   
     ...
     if(!is.null(segmentation.tile))
     {
        ...
        for (i in 1:length(list.rasters)) 
        {
           ...
           if (!is.null(ext))
           {
              ...
              zonal.stats.layer = exact_extract(s,segmentation.tile,fun=fun,force_df=TRUE,full_colnames=TRUE)   
